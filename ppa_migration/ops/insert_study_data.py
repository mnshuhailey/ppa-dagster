...
def fetch_running_no(cursor_sql, context):
    """Fetch prefix, year, and running_no from RunningNo table where idno is 7."""
    cursor_sql.execute("SELECT prefix, year, running_no FROM RunningNo WHERE idno = 7")
...
def update_running_no(cursor_sql, new_running_no, context):
    """Update the running_no in the RunningNo table where idno is 7."""
    cursor_sql.execute("UPDATE RunningNo SET running_no = ? WHERE idno = 7", (new_running_no,))
...
def insert_study_data(context, transformed_data):
    """Insert study data into SQL Server, skipping rows with null or empty snapshot_id."""
    if not transformed_data:
        context.log.warning("No data to insert or merge.")
        return

    base_dir = os.path.dirname(os.path.realpath(__file__))
    merge_data_query_path = os.path.join(base_dir, '../sql/insert_study_data.sql')

    # Load SQL query from file (assuming fixed number of placeholders)
    ...
    error_log = []
    batch_size = 5000  # Process data in batches
    ...
    with sqlserver_conn.cursor() as cursor_sql:
        ...
        all_asnaf_ids = [row[7] for row in transformed_data if row[7]]
        asnaf_snapshot_map = {}
        ...
        for data_batch in chunked_list(transformed_data, batch_size):
            try:
                ...
                for row in data_batch:
                    asnaf_id = str(row[7]).upper()  # Assuming row[7] contains AsnafID

                    # Get SnapshotID for the corresponding AsnafID or NULL if not found
                    snapshot_id = asnaf_snapshot_map.get(asnaf_id)

                    # Skip this row if snapshot_id is empty or null
                    if not snapshot_id:
                        error_log.append(f"AsnafID {asnaf_id} has no SnapshotID. Skipping this row.")
    ...
    write_log_file(context, error_log, 'error_insert_study_data', base_dir)
