...
def update_asnaf_snapshotid(context, transformed_data):
    if not transformed_data:
        context.log.warning("No data to update SnapshotID.")
        return

    base_dir = os.path.dirname(os.path.realpath(__file__))
    update_query_path = os.path.join(base_dir, '../sql/update_asnaf_snapshotid.sql')
    
    # Load SQL query from file
    update_query = read_sql_file(update_query_path)

    sqlserver_conn = context.resources.sqlserver_db
    context.log.info("Updating Asnaf SnapshotID based on Familyrelationship SnapshotID.")
    ...
    write_log_file(context, error_log, 'error_update_asnaf_snapshotid', base_dir)
